...
{
    public partial class frmScanBox1 : Form, IAcquireCallback//, ISave
    {
        ...
        private void frmScanBox_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (staMain.stcProjCfg.BatchAuto.Trim() == "Y")
                {
                    batchLabelStripLbl.Visible = false;
                    batchStripTxt.Visible = false;
                }
        ...
        private void frmScanBox1_Paint(object sender, PaintEventArgs e)
        {
            if (tvwBatch.Nodes.Count > 0)
            {
                staMain.sessionStop(false);
        ...
        private void frmScanBox_Resize(object sender, EventArgs e)
        {
            try
            {
                formResize();
        ...
        public void btnScan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (staMain.stcProjCfg.BatchAuto.Trim().ToUpper() == "N" && batchStripTxt.Text.Trim() == "")
                {
                ...
                if (staMain.stcProjCfg.BatchAuto.Trim().ToUpper() == "N")
                {
                    _batchCode = batchStripTxt.Text.Trim();
                }
                ...
                if (bValid)
                {
                    if (validateBoxInfo(out sMsg))
                    {
                        _boxNum = this.boxNoStripTxt.Text.Trim();
                        _boxRef = this.boxLabelStripTxt.Text.Trim();
                        if (staMain.stcProjCfg.BatchAuto.Trim().ToUpper() == "N" && batchStripTxt.Text.Trim() == string.Empty)
                        {
                            if (_boxRef.Trim() == string.Empty)
                                batchStripTxt.Text = _boxNum;
                            else
                                batchStripTxt.Text = _boxNum + "_" + _boxRef.Replace("-", "_");
                        }
		...
		private bool validateBoxInfo(out string sMsg)
        {
            bool bValid = true;
            sMsg = "";
            ...
            {
                if (!mGlobal.IsInteger(boxNoStripTxt.Text.Trim()))
                    sMsg = "Box number must be number!";
            }
            else
                sMsg = "Box number is required!";

            if (sMsg != "") bValid = false;
        ...
        private bool ReadFromImageBarcode(System.Drawing.Bitmap pBmp)
        {
            ...
            try
            {
                ...
                if (MDIMain.oImgCore.ImageBuffer.HowManyImagesInBuffer > 0)
                {
                    ...
                    //bmp = (Bitmap)(MDIMain.oImgCore.ImageBuffer.GetBitmap(MDIMain.oImgCore.ImageBuffer.CurrentImageIndexInBuffer));
            ...
            catch (Exception exp)
            {
                ...
                mGlobal.Write2Log("Decode barcode.." + exp.StackTrace.ToString());
        ...
        public void OnPostAllTransfers()
        {
            ...
            {
                btnCloseStrip.Enabled = true;
                mGlobal.Write2Log("Post all.." + ex.Message + Environment.NewLine + ex.StackTrace.ToString());
            }
        }
        ...
        public bool OnPostTransfer(Bitmap bit, string info)
        {
            ...
            try
            {
                ...
                if (sIsAddOn.Trim() == "")
                {
                    ...
                    else //Box 
                    {
                        ...
                        _batchCode = _batchCode.Trim() + "_" + boxNoStripTxt.Text.Trim();
                    }
                }                
            ...
            {
                mGlobal.Write2Log("Box Post Image.." + ScanStatusBar1.Text + ": " + icex.Message + Environment.NewLine 
            ...
            catch (Exception ex)
            {
                ...
                {
                    mGlobal.Write2Log("Scan Box Post.." + ScanStatusBar1.Text + ": " + ex.Message + Environment.NewLine
        ...
        public void OnPreAllTransfers()
        {
            ...
            //    if (saveBatchDB() == false)
            //    {
            //        sErrMsg = "Scan process failure!! Process aborted.";
            //        this.OnTransferCancelled();
            //    }
            //}            
        ...
        private bool saveDocScanDB(int pImgSeq = 0)
        {
            ...
            catch (Exception ex)
            {
                ...
                mGlobal.Write2Log("Save box docu.." + ex.Message);
                //throw new Exception(ex.Message);
            }
            ...
            if (_batchType.Trim().ToLower() == "set")
            {
                ...
                else
                {
                    if (staMain.stcProjCfg.BatchAuto.ToUpper() == "N")
                        ...
                        //staMain.loadDocuSetTreeview(this.tvwBatch, _batchType, _batchCode, "'" + _batchStatus + "'", "'S','A'", _scanStn, _scanUsr, pDateStart.ToString("yyyy-MM-dd HH:mm:ss"), pDateEnd.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private void frmScanBox_FormClosing(object sender, FormClosingEventArgs e)
        {
            //if (!btnScan.Enabled)
        ...
        private void frmScanBox_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (mGlobal.checkOpenedForms("frmNotification"))
        ...
        private void tvwBatch_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            try
            {
                ...
                else if (sep.Trim().ToLower() == "sep1")
                {
                    ...
                    else if (_batchType.ToLower().Trim() == "batch" || _batchType.ToLower().Trim() == "box") //Box.
                    {
                        mnuInsStrip.Enabled = false;
                        mnuImpStrip.Enabled = false;
                        mnuDeleteSetStrip.Enabled = true;
                ...
                else if (nodeInfo.Trim().ToLower() == "page")
                {
                    ...
                    if (e.Node.Tag.ToString().Trim().Split('|').Length > 1)
                    {
                        ...
                        if (sCurrSetNum.Trim() == sCurrDocType.Trim())
                            ScanStatusBar1.Text = "Set " + sCurrSetNum;
                        else if (sCurrDocType.Trim() != "")
                            ScanStatusBar1.Text = "Set " + sCurrSetNum + " : " + sCurrDocType;
                        else
                            ScanStatusBar1.Text = tvwBatch.Nodes[0].Text;
                    }
                    else
                            ...
                            if (bOK)
                            {
                                ...
                                if (bOK) //Reload all.
                                {
                                    ...
                                    if (sIsAddOn.Trim() != "")
                                    {
                                        ...
                                        //    loadImagesFrmDisk(_lCurrBatchRowid, "'1'", "'S','A','R'");
                ...
                if (bdr != null)
                {
                    ...
                    if (_batchType.Trim().ToLower() == "set")
                    {
                        ...
                        else
                        {
                            if (pBatchStatusIn.Trim().IndexOf("2") > 0)
                            {
                                drs = oDF.getIndexedDocFileBySetNumDB(_currScanProj, _appNum, _setNum, "'I'");
                            }
                            else
                                drs = oDF.getDocFileByBatchCodeDB(_currScanProj, _appNum, _batchCode, pDocStatusIn); //"'S','A'"
                                //drs = oDF.getDocFileBySetNumDB(_currScanProj, _appNum, _setNum, pDocStatusIn); //"'S','A'"
        ...
        private void setInfoForAddOn() //always go to last set and last doctype for addon.
        {
            try
            {               
        ...
        private void frmScanBox1_Activated(object sender, EventArgs e)
        {
            bool bValid = true;
            try
            {
                if (mGlobal.checkOpenedForms("frmReScanBox1"))
                    bValid = false;

                if (!bValid)
                {
                    MessageBox.Show("The Box Rescan windows is opened!");
                    enableScan(false);
                }
                else
                    enableScan(true);
