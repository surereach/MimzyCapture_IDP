...
{
    public partial class frmRescan1 : Form, IAcquireCallback//, ISave
    {
        ...
        private void frmReScan_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                enableScan(true); //enable MDIMain Scan.  
        ...
        private void frmReScan_Resize(object sender, EventArgs e)
        {
            try
            {
                formResize();
        ...
        private void frmRescan1_Paint(object sender, PaintEventArgs e)
        {
            if (tvwBatch.Nodes.Count > 0)
            {
        ...
        public void btnScan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (bValid)
                {
                    ...
                    bIsBlankPage = false;
                    bMultipleBarcodeOCR = false;
                    tvwBatch.Nodes.Clear(); //Commented if to facilitate duplicate doc type validation using lScanImg in OnPostTransfer. 
                    //>>
        ...
        private bool ReadFromImageBarcode(System.Drawing.Bitmap pBmp)
        {
            ...
            try
            {
                ...
                if (_oImgCore.ImageBuffer.HowManyImagesInBuffer > 0)
                {
                    ...
                    //bmp = (Bitmap)(_oImgCore.ImageBuffer.GetBitmap(_oImgCore.ImageBuffer.CurrentImageIndexInBuffer));
            ...
            {
                mGlobal.Write2Log("Decode barcode.." + ex.Message);
            }
            finally
            {
        ...
        public bool OnPostTransfer(Bitmap bit, string info)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    else if (_noSeparator.Trim() == "2" && _isDocSep)
                    {
                        ...
                        else if (_sepCnt > 1)
                        {
                            ...
                            //_docType = sCurrDocType; //Last doc. type. //Last doc. type. Commented for the continue of same doc type scanning, but will cause duplicate doc type notification.
                        }
                ...
                if (sIsAddOn.Trim() == "")
                {
                    ...
                    {
                        sErrMsg = "Post..batch code is blank in rescan.";
                        mGlobal.Write2Log(sErrMsg);
                        this.OnTransferCancelled();
                        return false;
                    }
            ...
            {
                mGlobal.Write2Log("Rescan Post Image.." + ScanStatusBar1.Text + ": " + icex.Message + Environment.NewLine 
            ...
            catch (Exception ex)
            {
                ...
                {
                    mGlobal.Write2Log("Rescan Post.." + ScanStatusBar1.Text + ": " + ex.Message + Environment.NewLine 
        ...
        private bool saveRescanDocIndexDB(string pDocuStatus, string pNewDocuStatus, string pNewFilename)
        {
            ...
            catch (Exception ex)
            {
                ...
                //mGlobal.Write2Log("Save doc idx.." + sSQL);
                bSaved = false;
                //throw new Exception(ex.Message);                            
            }
        ...
        private void loadBatchSetRescanTreeview(string pBatchStatusIn, string pStation = "", string pUserId = "")
        {
            string sSQL = "";
            DataSet dsTVw = null;
            string sTag = "";
            ...
            try
            {
                ...
                //Load Document.
                nc = 0;
                int nnc = 0; string sNodeText1 = ""; int nnnc = 0; string sNodeText2 = "";
                sTag = "";
                ...
                while (nc < tvwBatch.Nodes.Count)
                {
                    ...
                    while (nnc < tvwBatch.Nodes[nc].Nodes.Count)
                    {
                        sNodeText1 = tvwBatch.Nodes[nc].Nodes[nnc].Tag.ToString().Substring(5);  //Setnum.
                        if (_noSeparator.Trim() == "2")
                        {
                            iNodesCnt = tvwBatch.Nodes[nc].Nodes[nnc].Nodes.Count;
                            iLev = 2;
                        }
                        else
                        {
                            iNodesCnt = tvwBatch.Nodes[nc].Nodes.Count;
                            iLev = 1;
                            nnnc = nnc;
                        }
                        ...
                        while (nnnc < iNodesCnt)
                        {
                            ...
                            if (iLev == 2)
                                sNodeText2 = tvwBatch.Nodes[nc].Nodes[nnc].Nodes[nnnc].Tag.ToString().Split('|').GetValue(1).ToString();
                                ...
                                if (dsTVw.Tables.Count > 0)
                                {
                                    if (dsTVw.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        while (i < dsTVw.Tables[0].Rows.Count)
                                        {
                                            ...
                                            if (iLev == 2)
                                            {
                                                tvwBatch.Nodes[nc].Nodes[nnc].Nodes[nnnc].Nodes.Add(node);
                                                tvwBatch.Nodes[nc].Nodes[nnc].Nodes[nnnc].LastNode.Name = tvwBatch.Nodes[nc].Nodes[nnc].Nodes[nnnc].LastNode.FullPath;
                                            }
                                            else
                                            {
                                                tvwBatch.Nodes[nc].Nodes[nnc].Nodes.Add(node);
        ...
        private void frmReScan_FormClosing(object sender, FormClosingEventArgs e)
        {
            //if (!btnScan.Enabled)
        ...
        private void frmReScan_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (mGlobal.checkOpenedForms("frmNotification"))
        ...
        private void tvwBatch_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            try
            {
                ...
                else if (sep.Trim().ToLower() == "sep1")
                {
                    ...
                    else if (_batchType.Trim().ToLower() == "batch")
                    {
                ...
                {
                    if (_batchType.Trim().ToLower() == "batch")
                    {
            ...
            try
            {
                ...
                if (!bUpdate)
                {
                    if (pCurrImgIdx < lScanImg.Count)
                    {
                        ...
                        int i = 0;
                        while (i < _stcImgInfo.ImgFile.Length)
                        {
                            if (_stcImgInfo.ImgFile[i].Trim() == lScanImg[pCurrImgIdx].Trim())
                            {
                                _stcImgInfo.CurrImgIdx = (short)i;
                                sFile = _stcImgInfo.ImgFile[i].Trim();
                                break;
                            }
                            i += 1;
                        }
            ...
            try
            {
                ...
                if (tvwBatch.SelectedNode != null)
                {
                    ...
                    if (oImgCut != null)
                    {
                        ...
                        nodeInfo = node.Text.Trim().Substring(0, 4); //Doc Type?
                        sep = node.Tag.ToString().Trim().Substring(0, 4);
        ...
        private void frmRescan1_Activated(object sender, EventArgs e)
        {
            bool bValid = true;
            try
            {
                ...
                if (!bValid)
                {
                    MessageBox.Show("The Scan windows is opened!");
                    enableScan(false);
                }
                else
                    enableScan(true);
